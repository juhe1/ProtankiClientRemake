package services.alertservice
{
   ...
   public class Alert extends Sprite
   {
      ...
      protected function doLayout(param1:Event) : void
      {
         ...
         var _loc5_:int = 0;
         var _loc6_:DefaultButtonBase = null;
         ...
         if(this.hasButtons())
         {
            ...
            while(_loc5_ < this.labels.length)
            {
               ...
               {
                  _loc4_ = this.customButtons[_loc5_];
               }
               else
               {
                  _loc6_ = new DefaultButtonBase();
                  _loc4_ = _loc6_;
                  _loc6_.label = this.labels[_loc5_];
               }
      ...
      private function onKeyUp(param1:KeyboardEvent) : void
      {
         ...
         switch(this.labels.length)
         {
            ...
            case 2:
               ...
               {
                  _loc2_ = this.getFirstExistingLabel(this.getConfirmationButtonNames());
                  break;
               }
               if(AlertUtils.isCancelKey(param1.keyCode))
               {
                  _loc2_ = this.getFirstExistingLabel(this.getCancelButtonNames());
               }
               break;
            case 3:
               if(AlertUtils.isConfirmationKey(param1.keyCode))
               ...
               if(AlertUtils.isCancelKey(param1.keyCode))
               {
                  _loc2_ = this.getFirstExistingLabel(Vector.<String>([AlertAnswer.CANCEL]));
               }
         }
         if(_loc2_ != null)
         {
      ...
      private function dispatchClickEventForButtonWithLabel(param1:String) : void
      {
         ...
         while(_loc2_ < this.alertWindow.numChildren)
         {
            ...
            if(this.childHasLabel(_loc3_) && this.getChildLabel(_loc3_) == param1)
            {
      ...
      public function simulatePressedCancel() : void
      {
         ...
         {
            _loc1_ = this.getFirstExistingLabel(this.getCancelButtonNames());
         }
